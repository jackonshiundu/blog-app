...
export const RichTextComponents = {
  ...
  list: {
    ...
    number: ({ children }: any) => (
      <ul className="ml-10 list-decimal">{children}</ul>
    ),
  },
  block: {
    h1: ({ children }: any) => (
    ...
    blockquote: ({ children }: any) => (
      <h1 className="tborder-l-[#2b6777] border-l-4 pl-5 py-5 my-5">
        {children}
      </h1>
    ),
  },
    ...
    link: ({ children, value }: any) => {
      ...
      return (
        <Link
          href={value.href}
          rel={rel}
          className="underline decoration-[#2b6777] hover:decoration-black"
        >
          {children}
        </Link>
      );
